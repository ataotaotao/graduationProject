...
class TCA:
    ...
    def __init__(self, dim=5, kerneltype='linear', kernelparam=1, mu=1):
        '''
        Init function
...
#         print("n_src:n_tar", n_src, n_tar)
#         print ("shape:", np.shape(L), np.shape(L_st), np.shape(L_ts), np.shape(L_tt))
#         print L
        return L
...
#                 print np.shape(P)    (2000,)
                P = P.reshape((len(P), 1))
#                 print np.shape(P)    (2000,1)
                K = np.exp(
                    -1 * (np.tile(P.T, (n1, 1)) + np.tile(P, (1, n1)) - 2 * np.dot(x1, x1.T)) / (dim * 2 * kernelparam))
        # more kernels can be added
        return K

    def fit_transform(self, x_src, x_tar, x_tar_o=None, x_src_o=None):
        '''
...
#         print np.shape(D), np.shape(V)  # (2000,) (2000,2000)
...
#         print np.shape(x_src_tca)
        ...
        if x_src_o is not None:
            x_src_o_tca = x_src_o_tca[:, :self.dim]
        
        return x_src_tca, x_tar_tca, x_tar_o_tca, x_src_o_tca


if __name__ == '__main__':
    file_path = 'test_tca_data.csv'
    ...
    # example usage
    my_tca = TCA(dim=30)
    x_src_tca, x_tar_tca, x_tar_o_tca, X = my_tca.fit_transform(x_src, x_tar)
    print np.shape(x_src), np.shape(x_tar)
    print np.shape(x_src_tca), np.shape(x_tar_tca), np.shape(x_tar_o_tca)
