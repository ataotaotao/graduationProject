...
from excel_format_3 import Excel_format
...
class DEMO():
    def __init__(self,X_m_cal, y_m_cal, X_s_cal, y_s_cal, X_s_test, y_s_test, 
                 ...
                 max_components ,  X_m_std , y_m_std , X_s_std , y_s_std  , method_list , 
...
#         print np.shape(k1), np.shape(T_slave_test_)
    ...
    def pds(self,init_width,max_width ,n_folds_cal , n_max_components_cal):
        ...
        y_s_test = deal_4_1(self.y_s_test, self.i)
        y_s_cal = deal_4_1(self.y_s_cal, self.i)
        X_m_std , y_m_std , X_s_std , y_s_std = getStd(self.X_m_cal, self.X_s_cal, y_m_cal, y_s_cal, self.m)
...
#         y_sure_real = y_s_cal_[CalInd_master_std]
        ...
        y0_s_sure = y0_s[48:63,:]
        y1_s_sure = y1_s[48:63,:]
        y_sure_real = y_s_cal_[48:63,:]
...
#         return comp_best,RMSEP_y0,y0_pre,y_cal_pre , y0_s_test , y1_s_test, RMSEP_y0_
        ...
        return comp_best,RMSEP_y0,y0_pre,RMSEP_y0_,y0_pre_ , y0_s_test , y1_s_test , RMSEP_y0_
        
        
def exe_all(i , k , str , a , b , c , f , strsss):
    fileName = strsss
    ...
    print num
    for m in range(10 , 31 , 1):
        print m , f
        std_num.append(m)
        X_m_cal, y_m_cal, X_s_cal, y_s_cal, X_s_test, y_s_test, X_m_test, y_m_test , X_m_std , y_m_std , X_s_std , y_s_std = Dataset_KS_split_std(X_master , X_slave , y , num , std_num = m)
...
#         master_name = str(master_name_o) + '_' + str(np.shape(y_m_std)[0]) 
        ...
        demo=DEMO(X_m_cal, y_m_cal, X_s_cal, y_s_cal,
                  ...
                  y_m_test , max_folds,
                  max_components ,  X_m_std , y_m_std , X_s_std , y_s_std  ,method_list , RMSEC_list,RMSEP_list,ycal_predict_list,y_predict_list
                  ,comp_best_list , i,m)
...
#     ######## SBC
        if f == 0:
            ...
            RMSEC_sbc,RMSEP_sbc,yte_predict_sbc,comp_best_m_sbc, ycal_predict_sbc = demo.sbc(std_num = m)
            demo.append("SBC", RMSEC_sbc[0], RMSEP_sbc[0], ycal_predict_sbc, yte_predict_sbc, comp_best_m_sbc)
                     
            print RMSEC_sbc,  RMSEP_sbc , "sbc"
#      
        elif f == 1:
...
#      
#     ######## CCA  
            RMSEC_cca, RMSEP_cca,y_predict_cca,comp_best_cca, ycal_predict_cca = demo.cca(std_num = m)
            demo.append("CCA", RMSEC_cca[0], RMSEP_cca[0], ycal_predict_cca, y_predict_cca, comp_best_cca)
                     
            print RMSEP_cca , "cca" , RMSEC_cca
#       ######## PDS
        elif f == 2:
            RMSEC_pds,best_width_pds,RMSEP_pds,y_predict_pds,RMSECV_list_pds,comp_best_pds, ycal_predict_pds = demo.pds(init_width,max_width,n_folds_cal , max_components_cal)
            demo.append( "PDS" , RMSEC_pds[0] , RMSEP_pds[0] ,ycal_predict_pds , y_predict_pds , comp_best_pds )
       
            print RMSEP_pds , "RMSEP_PDS"
    result_dict = {
            "RMSEP_list":RMSEP_list,
            "RMSEC_list":RMSEC_list,
            "std_num_list":std_num
            
        }
    excel_result.write_excel(master_name_o, result_dict )
if __name__ == '__main__':

    
#     exe_all(0,0 , "0-SBC" , 1, 3, 5,0)
#     exe_all(1,0 , "1-SBC" , 1, 3, 5,0)
#     exe_all(2,0 , "2-SBC" , 1, 3, 5,0)
#     exe_all(0,0 , "0-CCA" , 1, 3, 5,1)
#     exe_all(1,0,"1-CCA",1,2,3,1)
#     exe_all(2,0,"2-CCA",1,2,3,1)
#     exe_all(0,0,"0-PDS" , 1, 3, 5,2)
#     exe_all(1,0,"1-PDS",1,2,3,2)
#     exe_all(2,0,"2-PDS",1,2,3,2)
...
#     exe_all(1,0 , "1-SBC" , 1, 3, 5,0)
#     exe_all(2,0 , "2-SBC" , 1, 3, 5,0)
#     exe_all(3,1 , "3-SBC" , 1, 3, 5,0)
#     exe_all(0,0 , "0-CCA" , 1, 3, 5,1)
#     exe_all(1,0,"1-CCA",1,2,3,1)
#     exe_all(2,0,"2-CCA",1,2,3,1)
#     exe_all(3,1,"3-CCA",1,2,3,1)
    exe_all(0,1,"0-CCA" , 1, 3, 5,1,'./result/result_yaopian.xlsx')
#     exe_all(0,1,"0-PDS" , 1, 3, 5,2,'./result/result_yaopian.xlsx')
#     exe_all(1,1,"1-PDS",1,2,3,2,'./result/result_yaopian.xlsx')
#     exe_all(2,1,"2-PDS",1,2,3,2,'./result/result_yaopian.xlsx')
# #     exe_all(3,1,"3-PDS",1,2,3,2)
# 
#     exe_all(0,0,"0-PDS" , 1, 3, 5,2,'./result/result_yaopian_no.xlsx')
#     exe_all(1,0,"1-PDS",1,2,3,2,'./result/result_yaopian_no.xlsx')
#     exe_all(2,0,"2-PDS",1,2,3,2,'./result/result_yaopian_no.xlsx')
